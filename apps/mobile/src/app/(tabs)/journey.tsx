...
import { ApiError, ApiNetworkError, fetchJourney, runAgent } from '@/lib/api';
...
function openFood(record: FoodRecord) {
  router.push({ pathname: '/record/[kind]', params: { kind: 'food', id: record.id, name: record.name, energy: String(record.energy_kcal), meal: record.meal_type, detail: record.detail ?? '', portion: record.portion_amount == null ? '' : String(record.portion_amount), portionUnit: record.portion_unit ?? '', timestamp: record.recorded_at } });
}

function openActivity(record: ActivityRecord) {
  router.push({ pathname: '/record/[kind]', params: { kind: 'activity', id: record.id, name: record.name, energy: String(record.energy_kcal), duration: String(record.duration_minutes), intensity: record.intensity, detail: record.note ?? '', timestamp: record.recorded_at } });
}

function openWeight(record: WeightRecord) {
  router.push({ pathname: '/record/[kind]', params: { kind: 'weight', id: record.id, weight: String(record.weight_kg), detail: record.note ?? '', timestamp: record.measured_at } });
}
...
function DayCard({ day }: { day: JourneyDay }) {
  ...
  return <Card>
    ...
    <Text style={[styles.meta, { color: theme.colors.textMuted }]}>摄入 {formatNumber(day.intake_kcal)} · 运动 {formatNumber(day.activity_kcal)} · 净结余</Text>
...
export default function JourneyScreen() {
  ...
  const journey = useQuery({ queryKey: ['journey', range], queryFn: () => fetchJourney(range) });
  const summary = useMemo(() => {
    const days = journey.data?.items ?? [];
    return days.reduce((acc, day) => ({ intake: acc.intake + day.intake_kcal, activity: acc.activity + day.activity_kcal, records: acc.records + day.food_records.length + day.activity_records.length + day.weight_records.length }), { intake: 0, activity: 0, records: 0 });
  }, [journey.data]);
  ...
  async function generateWeeklySummary() {
    ...
    try { setWeekly(await runAgent('请生成我的本周总结')); }
  ...
  return (
    <ScreenShell eyebrow="JOURNEY / 回看" title="看见每一步">
      {!sync.isOnline && <Notice tone="warning">当前显示最近一次成功加载的数据；编辑或删除记录需要联网。</Notice>}
      ...
      {journey.isLoading ? <LoadingState label="正在整理 Journey…" /> : journey.isError ? <ErrorState message="Journey 暂时无法加载" onRetry={() => void journey.refetch()} /> : journey.data ? (
        <>
          <View style={styles.metrics}><Metric label="总摄入" value={`${formatNumber(summary.intake)} kcal`} accent={theme.colors.food} /><Metric label="总运动" value={`${formatNumber(summary.activity)} kcal`} accent={theme.colors.primaryStrong} /><Metric label="记录" value={`${summary.records} 条`} /></View>
          <Card style={{ backgroundColor: theme.colors.hero }}>
            <SectionTitle>趋势速览</SectionTitle>
            <Text style={[styles.meta, { color: theme.colors.textMuted }]}>柱长表示每日净结余的绝对值，颜色区分正负。</Text>
            {(journey.data.items.slice(0, 7)).map((day) => <View key={day.date} style={styles.trendRow}><Text style={[styles.trendDate, { color: theme.colors.textMuted }]}>{day.date.slice(5)}</Text><View style={[styles.track, { backgroundColor: theme.colors.surface }]}><View style={[styles.bar, { width: `${Math.max(4, Math.abs(day.net_kcal) / maxNet * 100)}%`, backgroundColor: day.net_kcal > 0 ? theme.colors.food : theme.colors.primaryStrong }]} /></View><Text style={[styles.trendValue, { color: theme.colors.text }]}>{formatNumber(day.net_kcal)}</Text></View>)}
          </Card>
          <Card>
            <SectionTitle action={<Button variant="ghost" loading={weeklyPending} disabled={!sync.isOnline} onPress={() => void generateWeeklySummary()}>生成</Button>}>Agent 周总结</SectionTitle>
            <Text style={[styles.summary, { color: theme.colors.text }]}>{weekly?.answer ?? `最近记录了 ${summary.records} 条，摄入 ${formatNumber(summary.intake)} kcal，运动消耗 ${formatNumber(summary.activity)} kcal。`}</Text>
